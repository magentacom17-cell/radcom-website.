      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                selectedCategory === cat.id
                  ? 'bg-[#F5A623] text-slate-950 font-bold shadow-md shadow-amber-500/10'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredItems.map((item, index) => (
            <div
              key={item.id}
              className="group cursor-pointer rounded-2xl overflow-hidden border border-slate-800 bg-slate-900 hover:border-amber-500/50 transition-all flex flex-col hover:-translate-y-1 shadow-lg"
            ...
            >
              <div className="relative aspect-4/3 overflow-hidden bg-slate-950">
                ...
                <div className="absolute inset-0 bg-slate-950/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <div className="p-2.5 rounded-full bg-[#F5A623] text-slate-950 shadow-xl">
              ...
              <div className="p-3.5 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-2 pt-2 border-t border-slate-800/80 flex items-center justify-between text-[10px] text-slate-400">
                  ...
                  <span className="text-amber-400 font-semibold group-hover:underline">Detail &rarr;</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative max-w-2xl w-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <div>
                <span className="text-[11px] font-bold text-amber-400 uppercase tracking-wider">{activeItem.category}</span>
